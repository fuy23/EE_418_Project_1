    ...
    methods
        ...
        function [obj, curr_avg_offset_us, curr_acc_offset_us] = estimate_offset(obj, a)
            ...
            if strcmp(obj.mode, 'state-of-the-art')
                ...
                sum = 0;
                for i = 2:obj.N
                    sum = sum + a(i) - (a(1) + (i - 1) * prev_mu_T_sec);     
                end
                curr_avg_offset_us = sum / (obj.N - 1);
        ...
        function [obj, curr_error] = update_clock_skew(obj, curr_avg_offset_us, curr_acc_offset_us)
           ...
           curr_G = (lambda^(-1) * prev_P * time_elapsed_sec) / (1 + lambda^(-1) * time_elapsed_sec^2 * prev_P);
           curr_P = lambda^(-1) * (prev_P - curr_G * time_elapsed_sec * prev_P);
           curr_skew = prev_skew + curr_G * curr_error;
           % ====================== End of Your Code =========================
